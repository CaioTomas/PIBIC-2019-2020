...
\section{Grupos de permutação}
	...
	\label{lema identidade permutacoes}
        Se $e = \beta_{1}\beta_{2}\cdots\beta_{r}$, com $\beta$ sendo
	...
	subgrupo de $S_n$, como demonstrado abaixo.
	%
	\begin{theorem}
...
\section{Isomorfismos}
	...
	\end{definition}
	%
	\par\vspace{0.3cm} Na Definição \eqref{def isomorfismo}, no lado
	...
	\end{proof}
	%
	\par\vspace{0.3cm} Os isomorfismos têm algumas propriedades, 
	listadas a seguir nos Teoremas \eqref{isomorfismos em elementos} 
	e \eqref{isomorfismos em grupos}.
	%
	\begin{theorem}
	\label{isomorfismos em elementos}
		...
		\begin{enumerate}
			...
			\item $\forall n\in\N, a\in G, \phi (a^n) = [\phi (a)]^n$;
	...
	\begin{proof}
		...
		é fechado, $\langle \phi(a)\rangle \subseteq \overline{G}$. 
		...
		%
		\par\vspace{0.3cm} Agora, seja 
		$\overline{G} = \langle \phi(a)\rangle$. Como $G$ é fechado,
		...
		%
		\par\vspace{0.4cm}
		\textbf{7.} Como $|a| = |\phi(a)|$ para todo $a\in G$, então 
	...
	\begin{proof}
		...
		\eqref{isomorfismos em elementos}, sabemos que $ab = ba$ se, e
		...
		Teorema \eqref{isomorfismos em elementos}, que diz que 
		...
		$Z(G) = \{ z|gz = zg, \forall g\in G \}$. Daí, 
		$\phi(Z(G)) = 
		\{ \underbrace{\phi(z)}_{\in \overline{G}} | \phi(g)\phi(z) 
		= \phi(z)\phi(g), \forall \phi(g)\in\overline{G}\}$, que é, 
		por definição, $Z(\overline{G})$.
	\end{proof}
	%
	\par\vspace{0.3cm} Na Definição \eqref{def isomorfismo}, nada nos
	...
	\end{deff}
	%
	\par\vspace{0.3cm} Além disso, podemos ainda definir um tipo 
	...
	\end{deff}
	%
	\par\vspace{0.3cm} Um fato interessante de $\aut(G)$ e $\inn(G)$ 
	é o seguinte.
	%
	\begin{theorem}
	...
	\end{proof}
	%
	\par\vspace{0.3cm} Por exemplo, se tomarmos 
	...
	tal que $\phi(\beta) = \alpha$, a saber, $\beta = \overline{\alpha}$.
	...
	é automorfismo.
	%
	\par\vspace{0.3cm} Outro exemplo é o conjunto de automorfismos
	...
	\end{equation*}
	%
	De fato, para tal basta mostrarmos que 
	...
	\begin{proof}
		...
		$\alpha$. Como $a(k) = k\alpha(1)$, $T$ é injetora, pois se 
	...
	Pela propriedade 4 do Teorema \eqref{isomorfismos em elementos}, 
	...
	%
	\begin{remark}
		Pelo Teorema \eqref{automorfismos de Z_n}, 
...
\label{sec-classes-laterais}
	...
	\end{proof}
	%
	\par\vspace{0.3cm} Com a Definição \eqref{def classes laterais} e o 
	Lema \eqref{propriedades}, podemos enunciar o Teorema \eqref{lagrange}.
	%
	\begin{theorem}
	\label{lagrange}
	...
	\begin{proof}
		...
		Lema \eqref{propriedades}, $a\in aH$. Logo, temos $aH = H = a_iH$ 
		e, daí:
		%
		\begin{align*}
		...
		%
	\end{proof}
	\par\vspace{0.3cm} Alguns resultados seguem como consequência imediata 
	do Teorema \eqref{lagrange}.
	%
	\begin{corollary}
	\label{c2}
	...
	\begin{proof}
		Pelo Corolário \eqref{c2}, $|G| = |a| k, k\in\mathbb{Z}_{+}^{*}$.
	...
	\begin{proof}
		...
		%
		\par\vspace{0.3cm} Se $r = 0$, então $p|a$ e é claro que 
		$r^p\pmod p = r\pmod p$.
		%
		...
		Corolário \eqref{c4}, temos que $r^{p-1}\pmod p = 1$ e,
	...
	\end{proof}
	%
	\par\vspace{0.3cm} Um exemplo interessante de isomorfismo é o fato 
	de que $S_3\cong GL(2,\mathbb{Z}_2)$. De fato, seja
	...
	%
	\begin{proof}
		Pelo Teorema \eqref{lagrange}, $|G|/|\stab_G(i)|$ é o número de
		...
		Lema \eqref{propriedades}, $\alpha^{-1}\beta\in\stab_G(i)$, ou seja, 
		...
		Lema \eqref{propriedades}, $\alpha\stab_G(i) = \beta\stab_G(i)$. 
		Logo, $T$ é injetiva.
		%
	...
	\begin{proof}
		...
		mas não necessariamente particionam igualmente.  
	\end{proof}
	%
	\begin{theorem}
	...
	\begin{proof}
		...
		$S_4$ (pela propriedade 5 do Teorema \eqref{isomorfismos em grupos}).
		%
...
\label{sec-prod-direto-grupos}
	...
	%
	\begin{proof}
		Sejam $a_i, b_i\in G_i$. Da Definição \eqref{def prod direto externo},
		sabemos que:
		%
		\begin{align*}
		...
		elemento $(a_1, a_2, \dots, a_n)$ em $H$ é 
		$(a_1^{-1}, a_2^{-1}, \dots, a_n^{-1})$.
	\end{proof}
	%
	\begin{remark}
		Segue da Definição \eqref{def prod direto externo} que 
	...
	\begin{proof}
		...
		$|a| = |b| = |ab| = 2$, pelo Teorema \eqref{lagrange}. 
		Logo, podemos definir o mapeamento 
		$e\to (0,0)$, $a\to (1,0), b\to (0,1)$, $ab\to (1,1)$, que é um
	...
	\label{ordem}
		...
		%
		\begin{equation*}
		\bigoplus_{i \leq n} G_i = \{(g_1, g_2, \dots , g_n)| g_i\in G_i \}. 
		\end{equation*}
		%
		Então,
	...
	%
	\begin{proof}
		Para $n = 2$ temos o Teorema \eqref{crit}. Suponha que o corolário
		...
		\end{equation*}
		%
		\par\vspace{0.3cm} Sejam $|G_i| = n_i$ para 
	...
	%
	\begin{proof}
		Do Corolário \eqref{C1}, sabemos que
	...
	%
	\begin{remark}
        Em \cite{Livro-do-Fraleigh}, o Teorema \eqref{crit} e o 
        Corolário \eqref{C1} são enunciados (de certo modo) como o 
	...
	\begin{proof}
		...
		\end{align*}
		%
		($\Rightarrow$) Se $a\pmod {st} = b\pmod {st}$, então $st|a-b$. 
		Logo:
		%
		\begin{align*}
            \begin{cases}
	...
	\begin{proof}
		...
		\begin{align*}
            \begin{cases}
                ...
                x\pmod t = y\pmod t
            \end{cases} 
        \overset{\text{Lema \eqref{lema1}}}{\iff } x\pmod {st} = y\pmod {st} 
		\end{align*}
		%
		...
		o que implica que $\mdc(s,aq_2) = 1$ pelo Lema \eqref{lema2}. 
		Além disso, $\mdc(s,t) = 1$, logo $\mdc(s,atq_2) = 1$ pelo 
		Lema \eqref{lema2} novamente.
		%
		...
		$\mdc(t,bq_1) = 1$ pelo Lema \eqref{lema2} e, como $\mdc(t,s) = 1$,
		então $\mdc(t,bsq_1) = 1$ pelo Lema \eqref{lema2} novamente.
		%
		...
		\begin{align*}
            ...
            \begin{cases}
                ...
                y\pmod s = y\pmod s
            \end{cases} 
            \overset{\text{Lema } \eqref{lema1}}{\iff } x\pmod {st} 
            = y\pmod{st} 
		\end{align*}
		%
		...
		logo $\mdc(s, tq_2) = 1$ pelo Lema \eqref{lema2}.
		%
		...
		logo $\mdc(t, bsq_1) = 1$ pelo Lema \eqref{lema2}.
		%
		...
		\begin{align*}
		    z\text{ }\mathrm{mod} s 
		    = tq_2\text{ }\mathrm{mod} s 
		    = 1 - sq_1 \text{ }\mathrm{mod} s = 1
		\end{align*} 
		%
		...
		$x\text{ }\mathrm{mod} t = 1 = y\text{ }\mathrm{mod} t$. 
		Logo, temos:
		%
		\begin{align*}
            \begin{cases}
            ...
            \end{cases} 
            \overset{\text{Lema }\eqref{lema1}}{\iff}x\text{ }\mathrm{mod} {st} 
            = y\text{ }\mathrm{mod} {st} 
		\end{align*}
		%
		...
		\begin{align*}
		    z\text{ }\mathrm{mod} t 
		    = sq_1\text{ }\mathrm{mod} t 
		    = 1 - tq_2 \text{ }\mathrm{mod} t 
		    = 1
		\end{align*} 
		%
		...
		\begin{align*}
		\phi_3(z) 
		&= (sq_1 + btq_2)\text{ }\mathrm{mod} s \\ 
		&= btq_2\text{ }\mathrm{mod} s \\ 
		&= (b - bsq_1)\text{ }\mathrm{mod} s \\ 
		&= b\text{ }\mathrm{mod} s
		\end{align*}
		%
	...
	%
	\begin{proof}
		Para o caso $k = 2$ temos o Teorema \eqref{produto direto}. Suponha então que o corolário 
...
\label{sec-subgrupos-normais}
	...
	\end{proof}
	%
	\par\vspace{0.3cm} Note que o Teorema \eqref{condicao} é uma versão mais fraca da propriedade 8 
	do Lema \eqref{propriedades}. Por exemplo, a partir do Teorema \eqref{condicao}, temos que todo 
	...
	%
	\begin{remark}
		Por fim, a contra-positiva do Teorema \eqref{teorema G/Z} é mais usada, isto é, se $G$ não é abeliano,
		$G/Z(G)$ não é cíclico. Por exemplo, segue dessa sentença e do Teorema \eqref{lagrange} que 
		um grupo não abeliano de ordem $pq$, com $p, q$ primos, deve ter um centro trivial, pois como todo 
		grupo de ordem prima é cíclico, então $|G/Z(G)| \overset{!}{=} pq$ (pois $G/Z(G)$ não pode ser cíclico), 
		o que implica $|Z(G)| = 1$. 
	\end{remark}
	%
	\begin{theorem}
	...
	\begin{proof}
		...
		tem ordem $q$, pelo Teorema \eqref{lagrange}.
	...
	%
	\begin{proof}
		Primeiro vamos mostar que a normalidade dos $H's$, junto com a segunda condição da 
		Definição \eqref{def prod direto interno}, implica na comutatividade dos $h's$. 
		...
		$h_1\cdots h_n$, com $h_i\in H_i$. Pela condição 1 da Definição \eqref{def prod direto interno}, 
	...
	%
	\begin{proof}
		Do Teorema \eqref{classificacao grupos de ordem p2}, segue que $G$ é isomorfo a 
...
\label{sec-homomorfismos}
	...
	%
	\begin{remark}
		Note que, das Definições \eqref{def isomorfismo} e \eqref{def homomorfismo}, temos que um 
	...
	\begin{proof}
		...
		\begin{align*}
		    \phi(a) = \phi(b) \iff (\phi(b))^{-1}\phi(a) 
		            = \overline{e} \iff b^{-1}a\in \Ker\phi \overset{\text{Lema }\eqref{propriedades}}{\iff} 
		            a\Ker\phi = b\Ker\phi
		\end{align*}
		%
		...
		\begin{align*}
		    ...
		    = g\Ker\phi \overset{\text{Lema }\eqref{propriedades}}{\iff}
	...
	\begin{proof}
		...
		\par\vspace{0.3cm}\hspace{17pt}\textbf{8.} Vamos usar o Teorema \eqref{condicao}. 
	...
	\end{proof}
	%
	\par\vspace{0.3cm} A propriedade 8 do Teorema \eqref{homomorfismos em subgrupos} tem um caso especial
	...
	de $\mathbb{Z}_{12}$ em $\mathbb{Z}_{30}$. Pela propriedade 2 do Teorema \eqref{homomorfismos em elementos},
	sabemos que os homomorfismos são completamente definidos pela imagem de 1, ou seja, 
	se $1$ é mapeado em $a$, então $x$ é mapeado em $xa$. Pelo Teorema \eqref{lagrange}, $|a|$ divide 30 e,
	pela propriedade 3 do Teorema \eqref{homomorfismos em elementos}, $|a|$ divide 12. Daí, 
	...
	\par\vspace{0.3cm} Outro exemplo é entre $S_n$ e $\mathbb{Z}_2$. A função de $S_n$ em $\mathbb{Z}_2$ 
	...
	%
	\begin{proof}
		Seja $\psi$ tal correspondência. Pela propriedade 5 do Teorema \eqref{homomorfismos em elementos}, 
		...
		\end{align*}
		%
		\par\vspace{0.3cm} Como todo $\phi(g)$ em $\phi(G)$ é atingido por algum elemento de $G/\Ker\phi$, 
		a saber $g\Ker\phi$, então $\psi$ é sobrejetora e, portanto, isomorfismo.
	\end{proof}
	%
	\par\vspace{0.3cm} Usando o Teorema \eqref{lagrange}, a propriedade 1 do 
	Teorema \eqref{homomorfismos em subgrupos} e o Teorema \eqref{primeiro teorema de isomorfismo}, 
	...
	%
	\begin{proof}
		Do Teorema \eqref{primeiro teorema de isomorfismo}, sabemos que $G/\Ker\phi\cong\phi(G)$, 
		ou seja, $|G/\Ker\phi| = |\phi(G)|$. Daí, sabemos que $|G| = |\phi(G)||\Ker\phi|$, 
		portanto $|\phi(G)|$ divide $|G|$. Agora, usando a propriedade 1 do 
		Teorema \eqref{homomorfismos em subgrupos}, sabemos que $\phi(G)$ é subgrupo de $\overline{G}$.
		Consequentemente, pelo Teorema \eqref{lagrange}, $|\phi(G)|$ divide $|\overline{G}|$.
	\end{proof}
	%
	\par\vspace{0.3cm} Com o Teorema \eqref{primeiro teorema de isomorfismo} podemos mostrar que
	$\mathbb{Z}/\langle n \rangle\cong \mathbb{Z}_n$. Para isso, considere o mapeamento $\phi$ definido por
	$\phi(m) = m\text{ }\mathrm{mod} n$. Podemos ver que todos os múltiplos de $n$ são levados na identidade, 
	ou seja, $\Ker\phi = \langle n \rangle$. Então, do Teorema \eqref{primeiro teorema de isomorfismo},
	...
	\begin{proof}
		...
		por definição). Daí, do Teorema \eqref{primeiro teorema de isomorfismo}, temos $N(H)/C(H)$ 
	...
	\begin{proof}
		...
		pela propriedade 2 do Lema \eqref{propriedades}, é verdade se, e só se, $g\in N$. Logo, 
		todo elemento do núcleo é, na verdade, elemento de $N$ e vice-versa. Portanto, $N = \Ker\gamma$.
	\end{proof}
	%
	\par\vspace{0.3cm} Assim como foi o caso com grupos quocientes, as imagens homomórficas de um grupo 
	...
	\par\vspace{0.3cm} Por exemplo, se $G$ tem ordem 60 e uma imagem homomórfica cíclica de ordem 12, então
	sabemos (pelas propriedades 5, 7 e 8 do Teorema \eqref{homomorfismos em subgrupos}) que $G$ tem subgrupos
	...
	\par\vspace{0.3cm} Apesar de serem parecidos, homomorfismos e isomorfismos desempenham papeis diferentes.
	Enquanto isomorfismos nos permitem olhar de um modo diferente para um grupo, homomorfismos agem como
	ferramentas investigativas. Em certas áreas de Teoria dos Grupos (especialmente em Física e Química),
	...
	\end{equation*}
	%
	\par\vspace{0.3cm} Se existe um subgrupo $Q$ de $G$ tal que $G\to G/N$ mapeia $Q$ isomorficamente 
	...
	\end{equation*}
	%
	\par\vspace{0.3cm} onde $q$ deve ser o único elemento de $Q$ sendo mapeado em $gN\in G/N$, e 
	...
	\end{deff}
	%
	\par\vspace{0.3cm} Note que $Q$ não precisa ser normal em $G$. Quando $G$ é o produto semidireto 
	de $N$ e $Q$, escrevemos $G = N \rtimes Q$ ou ainda $G = N\rtimes_{\theta} Q$.
	
	\par\vspace{0.3cm} Se um grupo $G$ é produto semidireto de dois grupos cíclicos, então 
	...
	\par\vspace{0.3cm} Por exemplo, em $D_n$, $n\geq2$, sejam $C_n = \langle r \rangle$ e 
	...
	\par\vspace{0.3cm} O grupo alternado $A_n$ é normal em $S_n$ (pois tem índice 2) e 
	$C_2 = \left\{ (12) \right\}$ é isomorfo a $S_n/A_n$. Portanto, $S_n = A_n\rtimes C_2$
	
	\par\vspace{0.3cm} Um grupo cíclico de ordem $p^2$, $p$ primo, não pode ser escrito como 
	produto semidireto, uma vez que tem apenas um subgrupo de ordem $p$ e seriam necessários 2.
	
	\par\vspace{0.3cm} Seja $G = GL(n,\F)$. Seja $B$ o subgrupo de matrizes triangulares superiores 
	...
	\end{equation*}
	%
	\par\vspace{0.3cm} e que essa terna determina $G$. Agora, vamos mostrar que toda terna 
	...
	\end{equation*}
	%
	\par\vspace{0.3cm} é um isomorfismo de grupos se, e só se, $\theta$ é o homomorfismo trivial 
	...
	\end{equation*}
	%
	\par\vspace{0.3cm} O grupo $G = N\rtimes_{\theta} Q$ é um grupo de ordem $p^3$ com geradores $a,b,c$ 
	e relações
	%
	\begin{equation}
	\label{sem p^2}
	...
	\end{equation*}
	%
	\par\vspace{0.3cm} Para um primo ímpar $p$, um grupo não comutativo (ou não abeliano) de ordem $p^3$ 
